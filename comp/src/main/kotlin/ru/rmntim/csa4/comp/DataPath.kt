...
class ALU(
    ...
    fun output(microcode: Array<Signal>): Int {
        ...
        val leftOperand =
            if (Signal.ALULeftOPDataStack in microcode) {
...
class IOController {
    ...
    private fun generateIOOutputLog(
        port: Int,
        value: Int,
    ): String = "I/O OPERATION OCCURS (OUT): port: $port <--- value: $value"

    private fun generateIOInLog(
        port: Int,
        value: Int,
    ): String = "I/O OPERATION OCCURS (IN): port: $port ---> value: $value"

    fun input(port: Int): Int {
        val input =
            if (connectedDevices[port]?.inputBuffer?.isNotEmpty() ?: throw NoDeviceException(port)) {
...
class NoDeviceException(
    port: Int,
) : Exception("no device at port $port")
