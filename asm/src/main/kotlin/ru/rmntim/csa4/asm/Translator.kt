 ...
 */
private val INSTRUCTION_REGISTRY =
    mapOf(
        ...
        "lit" to
            InstructionConfig(Opcode.LIT, InstructionType.OPERAND_REQUIRED) { operand ->
                operand.toIntOrNull() != null || isValidLabel(operand) || isValidCharacterLiteral(operand)
            },
        "word" to
            InstructionConfig(Opcode.WORD, InstructionType.OPERAND_REQUIRED) { operand ->
                operand.toIntOrNull() != null || isValidLabel(operand) || isValidCharacterLiteral(operand)
            },
        "buf" to
            InstructionConfig(Opcode.BUF, InstructionType.OPERAND_REQUIRED) { operand ->
                operand.toIntOrNull() != null && operand.toInt() > 0
            },
        // Data declarations
        "string" to
            InstructionConfig(Opcode.STRING, InstructionType.DATA_DECLARATION) { operand ->
 ...
 */
@Suppress("CyclomaticComplexMethod") // It's not that bad
private fun parseCharacterLiteral(literal: String): Int? {
    ...
    return when {
        ...
        content.length == 2 && content.startsWith('\\') -> {
            @Suppress("MagicNumber") // ASCII values
            when (content[1]) {
                'n' -> 10 // newline
                't' -> 9 // tab
 ...
 */
private fun isValidCharacterLiteral(literal: String): Boolean = parseCharacterLiteral(literal) != null
...
class AssemblyLexer {
    ...
    private fun parseMacroDefinition(
        operand: String,
        lineNumber: Int,
    ): Token {
            ...
            when (val token = tokens[i]) {
                ...
                is Token.MacroCall -> {
                    ...
                    if (macro.parameters.size != token.arguments.size) {
                        throw SyntaxError(
                            "Macro '${token.name}' expects ${macro.parameters.size} arguments, " +
                                "got ${token.arguments.size}",
                            token.line,
                        )
                    }
    ...
    private fun tokenToSourceLine(token: Token): String =
        when (token) {
            ...
            is Token.Instruction ->
                "${token.opcode.name.lowercase()}${if (token.operand != null) " ${token.operand}" else ""}"
...
class InstructionGenerator {
    ...
    private fun generateSimpleInstruction(opcode: Opcode): List<LabelInstruction> =
        listOf(
            LabelInstruction(MemoryCell.Instruction(opcode)),
        )
}
...
class AssemblyParser {
    ...
    private fun validateInstructionPlacement(
        ...
        val config =
            INSTRUCTION_REGISTRY[instruction.opcode.name.lowercase()]
        ...
        when (currentSection) {
            Section.TEXT -> {
                ...
                if (config.type == InstructionType.DATA_DECLARATION ||
                    ...
                    throw SyntaxError(
                        "Data declaration '${instruction.opcode.name.lowercase()}' not allowed in .text section",
                        instruction.line,
                    )
                }
            }
            Section.DATA -> {
                ...
                if (config.type != InstructionType.DATA_DECLARATION &&
                    ...
                    throw SyntaxError(
                        "Instruction '${instruction.opcode.name.lowercase()}' " +
                            "not allowed in .data section",
                        instruction.line,
                    )
                }
            }
...
class AssemblyTranslator {
    ...
    @Suppress("SwallowedException") // We want it to crash
 ...
 */
private fun generateDebugOutput(
    program: Program,
    outputFile: String,
) {
...
private fun formatInstructionAsHex(instruction: MemoryCell): String =
    when (instruction) {
        is MemoryCell.Instruction -> {
            ...
            opcodeValue.toHexString()
        }
        is MemoryCell.OperandInstruction -> {
            val opcodeValue = instruction.opcode.ordinal
            val operandValue = instruction.operand
            "${opcodeValue.toHexString()} ${operandValue.toHexString()}"
        }
        is MemoryCell.Data -> {
            instruction.value.toHexString()
        }
    }
